...
;; (define (make-circle area) (circle 10 "solid" "red"))
...
;; Consumes two strings and concatenates them, placing the shorter string at the beginning
...
(check-expect (academic-standing "Leo" "Tolstoy" 4.0) "Leo Tolstoy is on the Dean’s list.")


(define (academic-standing first-name last-name GPA)
  (string-append (string-append first-name " ")
                 (string-append (string-append last-name " ") "is ")
                 (cond
                   [(> GPA 3.5) "on the Dean’s list."]
                   [(< GPA 2.0) "on academic probation."]
                   [else "making satisfactory academic progress."])))
